...
class ViewController: UIViewController {
    ...
    func calcTotal_Average(){
        ...
        lblTotal.text = "총점은 \(calcFunction.0)점입니다."
        lblAverage.text = "평균은 \(round(calcFunction.1 * 100) / 100)점입니다."
    }
    ...
    func summaryGrade(){
        ...
        if averageScore > Double(score[0]){
            lblKorean.text = "국어 점수는 \(score[0])점으로 평균보다 낮습니다."
        }else if averageScore < Double(score[0]){
            lblKorean.text = "국어 점수는 \(score[0])점으로 평균보다 높습니다."
        }else{
            lblKorean.text = "국어 점수는 \(score[0])점으로 평균과 동일합니다."
        }
        
        // 영어
        if averageScore > Double(score[1]){
            lblEnglish.text = "영어 점수는 \(score[1])점으로 평균보다 낮습니다."
        }else if averageScore < Double(score[1]){
            lblEnglish.text = "영어 점수는 \(score[1])점으로 평균보다 높습니다."
        }else{
            lblEnglish.text = "영어 점수는 \(score[1])점으로 평균과 동일합니다."
        }

        // 수학
        if averageScore > Double(score[2]){
            lblMathematics.text = "수학 점수는 \(score[2])점으로 평균보다 낮습니다."
        }else if averageScore < Double(score[2]){
            lblMathematics.text = "수학 점수는 \(score[2])점으로 평균보다 높습니다."
        }else{
            lblMathematics.text = "영수학어 점수는 \(score[2])점으로 평균과 동일합니다."
        }
